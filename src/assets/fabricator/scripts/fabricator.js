...
fabricator.dom = {
    ...
    menuArrow: document.querySelectorAll('.f-menu li .arrow')
};
...
fabricator.buildColorChips = function () {
	...
	for (var i = chips.length - 1; i >= 0; i--) {
        ...
        chips[i].style.backgroundColor = color;
...
(function () {
    ...
    if(window.location.pathname){
        var page = window.location.pathname.slice(1).split('.').shift();
        if(page){
            $('.'+page).addClass('arrow-active');
            $('.'+page).find('ul').addClass('show');
        }

    }

    $('.f-menu .arrow').on('click',function(){
